...
int main(){
	...
	int count = 0;
	printf("Client 2 - Process coming up.... \n");
	while(true){		
		//write r to request access to device to mediator
		char *c = "C2#A";
		send(socketfd,c,strlen(c),0);
		printf("Client 2 - Sent message to mediator for device access \n");
		...
		printf("Client 2 is sleeping for %d seconds\n",time);
		sleep(time);
		//write d to send message to mediator that use is over
		char *d = "C2#R";
		send(socketfd,d,strlen(d),0);
		printf("Client 2 - Sent message to mediator for device release \n");
